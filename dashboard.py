...
from steering import _steering_log, _steering_log_lock, send_steering
...
AGENT_COLORS = {
    ...
    "developpeur": "yellow",
    "qa": "red",
    "reviewer": "bright_blue",
    "spawner": "blue",
    "user": "bright_yellow",
    "steering": "bright_yellow",
}
...
def build_agent_panel(agent_name: str) -> Panel:
    ...
    prog_text = f"[{color}]{bar}[/{color}] {pct}"
...
def build_event_bus_panel() -> Panel:
    ...
    return Panel(table, title="Event Bus -- Live", border_style="blue", box=box.ROUNDED)
...
def build_memory_panel() -> Panel:
    ...
    if not mem:
        return Panel(
            "[dim]Memory not initialized[/dim]", title="Memory", box=box.ROUNDED
        )
...
def build_steering_panel() -> Panel:
    ...
    for entry in recent:
        tgt = entry["target"]
        tgt_label = (
            f"[bold cyan]{tgt.upper()}[/bold cyan]"
            if tgt != "all"
            else "[bold white]ALL[/bold white]"
        )
    ...
    lines.append("")
    lines.append(
        "[bold yellow]Press S to send steering guidance to agents[/bold yellow]"
    )
    content = "\n".join(lines)
    return Panel(
        Text.from_markup(content),
...
class SwarmDashboard:
    ...
    def _build_layout(self) -> Layout:
        ...
        header_text = (
            f"[bold]hackathon-ibm -- Multi-Agent Swarm[/bold]  |  "
            f"[cyan]{m:02d}:{s:02d}[/cyan]  |  "
            f"[green]Active: {active}[/green]  |  "
            f"[blue]Done: {done}/{len(AGENT_CONFIGS)}[/blue]  |  "
            f"[dim]Memory: {len(mem._docs) if mem else 0} entries[/dim]"
        )
