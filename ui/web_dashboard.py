...
async def get_messages():
    """Return the full cross-agent message history with read/unread status."""
    from core.state import _message_history, _message_history_lock

    with _message_history_lock:
        return JSONResponse(list(_message_history))


@app.get("/api/skills")
async def get_skills():
    """Return skills used per agent."""
    from core.state import _agent_skills_used, _skills_lock

    with _skills_lock:
        return JSONResponse(dict(_agent_skills_used))
...
async def steer_agents(request: Request):
    ...
    from agents.steering import send_steering as _send_steering

    _send_steering(message, target)
...
.inbox-msg.read{opacity:0.35;}
.inbox-msg .im-dir{font-size:9px;color:var(--text2);font-weight:600;margin-bottom:2px;text-transform:uppercase;letter-spacing:.4px;}

/* overlay comms read/unread */
.dcard.ov-msg-read{opacity:0.4;}
.dcard.ov-msg-unread{border-left:3px solid var(--accent);}
...
<div class="layout">
  ...
  <div class="panel-right">
    <div class="rtabs">
      ...
      <div class="rtab" onclick="switchRight('r-inbox',this)">Messages</div>
...
<div class="bottom-bar" id="bottom-bar">
  ...
  <select id="steer-target" style="display:none;background:var(--card);color:var(--text);border:1px solid var(--border);border-radius:6px;padding:4px 8px;font-size:12px;margin-right:6px;outline:none;cursor:pointer;">
...
<div id="overlay" class="overlay">
  ...
  <div class="ov-tabs">
    ...
    <div class="ov-tab" onclick="switchOvTab('ov-skills',this)">Skills</div>
  </div>
  <div class="ov-body">
    ...
    <div id="ov-skills" class="ov-sec"></div>
  </div>
</div>
...
/* ═══════════════════════════════════════════════════════════════
   STEER TARGET SELECTOR
   ═══════════════════════════════════════════════════════════════ */
var _steerOptionsCache='';
function _updateSteerOptions(){
  var names=Object.keys(agents).sort();
  var key=names.join(',');
  if(key===_steerOptionsCache) return;
  _steerOptionsCache=key;
  var sel=document.getElementById('steer-target');
  var cur=sel.value;
  sel.innerHTML='<option value="all">All agents</option>';
  for(var i=0;i<names.length;i++){
    var opt=document.createElement('option');
    opt.value=names[i];opt.textContent=(AGENT_ICONS[names[i]]||'')+' '+names[i];
    sel.appendChild(opt);
  }
  sel.value=cur||'all';
}
...
/* ═══════════════════════════════════════════════════════════════
   INBOXES  (full message history with read/unread)
   ═══════════════════════════════════════════════════════════════ */
function renderInboxes(){
  fetch('/api/messages').then(function(r){return r.json();}).then(function(msgs){
    ...
    if(!msgs||!msgs.length){
      var empty='<div class="empty">No messages yet</div>';
      if(renderCache.inboxes!==empty){el.innerHTML=empty;renderCache.inboxes=empty;}
      return;
    }
    /* Group by conversation pair (sorted by first message time) */
    var grouped={};
    for(var i=0;i<msgs.length;i++){
      var m=msgs[i];
      var key=[m.from,m.to].sort().join(' <-> ');
    ...
    for(var k=0;k<keys.length;k++){
      var pair=keys[k];
      var list=grouped[pair];
      var unread=list.filter(function(m){return !m.read;}).length;
      var badge=unread>0?'<span class="inbox-cnt" style="background:var(--accent);color:#fff">'+unread+'</span>':'<span class="inbox-cnt" style="opacity:0.5">0</span>';
      ...
      for(var j=0;j<list.length;j++){
        var m=list[j];
        var readCls=m.read?' read':'';
        var mt=m.timestamp?new Date(m.timestamp).toLocaleTimeString():'';
        var fromC=color(m.from);
        html+='<div class="inbox-msg'+readCls+'">'
          +'<div class="im-dir" style="color:'+fromC+'">'+esc(m.from)+' -> '+esc(m.to)+(mt?' | '+esc(mt):'')+'</div>'
          +'<div class="im-body">'+esc(m.message)+'</div></div>';
      }
      html+='</div></div>';
    }
...
function renderOverlayData(name){
  ...
  /* comms – fetch full message history and show all messages involving this agent */
  var commEl=document.getElementById('ov-comms');
  fetch('/api/messages').then(function(r){return r.json();}).then(function(msgs){
    var agentMsgs=msgs.filter(function(m){return m.from===name||m.to===name;});
    if(!agentMsgs.length){
    ...
    for(var i=0;i<agentMsgs.length;i++){
      var m=agentMsgs[i];
      var isRead=m.read;
      var isSent=m.from===name;
      var readCls=isRead?' ov-msg-read':' ov-msg-unread';
      var dirLabel=isSent?('To '+m.to):('From '+m.from);
      var dirIcon=isSent?'\u{1F4E4}':'\u{1F4E5}';
      var mt=m.timestamp?new Date(m.timestamp).toLocaleTimeString():'';
      var fromC=color(m.from);
      html+='<div class="dcard br-accent'+readCls+'"><div class="dcard-h"><div class="dl">'
        +'<span class="dlbl" style="color:'+fromC+'">'+dirIcon+' '+esc(dirLabel)+'</span>'
        +'</div><span class="dt">'+esc(mt)+'</span></div>'
        +'<div class="dcard-b">'+esc(m.message)+'</div></div>';
    }
  ...
  /* skills */
  var skillEl=document.getElementById('ov-skills');
  fetch('/api/skills').then(function(r){return r.json();}).then(function(allSkills){
    var list=allSkills[name]||[];
    var html='';
    if(!list.length){
      html='<div class="empty">No skills used yet</div>';
    } else {
      for(var i=0;i<list.length;i++){
        html+='<div class="dcard br-green"><div class="dcard-h"><div class="dl">'
          +'<span class="dlbl" style="color:var(--green)">\u{1F9E0} Skill</span>'
          +'</div></div>'
          +'<div class="dcard-b">'+esc(list[i])+'</div></div>';
      }
    }
    if(renderCache.overlaySkills!==html){
      skillEl.innerHTML=html;
      renderCache.overlaySkills=html;
    }
  }).catch(function(){});
}
  ...
  for(var i = 0; i < batch.length; i++){
    ...
    if(d.type === 'event'){
      ...
      if(!swarmRunning){
        ...
        document.getElementById('steer-target').style.display='inline-block';
      }
      /* keep steer-target dropdown in sync with known agents */
      _updateSteerOptions();
...
function handleSubmit(){
  ...
  if(!swarmRunning){
    ...
    /* --- STEERING MODE --- */
    var steerTarget=document.getElementById('steer-target').value||'all';
    fetch('/api/steer',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:text,target:steerTarget})})
      .then(function(r){return r.json();})
      .then(function(){
        input.value='';input.style.height='auto';
        btn.disabled=false;
...
/* Message history poll */
setInterval(renderInboxes, 3000);
</script>
</body>
</html>
"""
