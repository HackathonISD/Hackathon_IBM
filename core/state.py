...
# Full message history (sent + read tracking for dashboard display)
_message_history: list[dict] = []
_message_history_lock = threading.Lock()

# Skills invoked per agent (agent_name -> list of skill names)
_agent_skills_used: dict[str, list[str]] = {}
...
try:
    import ui.web_dashboard as _web_dashboard_module
...
- SPAWNER: Can add temporary specialist agents when the project needs domain expertise beyond the core team
...
- If the project needs missing domain expertise, any core agent may ask the spawner for a temporary specialist agent instead of guessing
- Fix root causes across boundaries: if a failure spans code, tests, UI, runtime, or DevOps, coordinate until the whole flow works instead of stopping at your own layer
