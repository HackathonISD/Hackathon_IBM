...
from agents.agent_config import AGENT_CONFIGS, AGENT_PREREQS
from agents.agent_tools import (
    make_communication_tools,
    make_specialist_request_tool,
    make_stuck_tool,
)
from agents.steering import AgentTracePlugin, UserSteeringPlugin, UserSteeringRegistry
...
def _summarize_history(agent_name: str, messages: list[dict]) -> str:
    ...
    """
    from core.state import _llm_provider  # late import to pick up runtime value
...
async def run_agent(agent_name: str, specs: str, loop: asyncio.AbstractEventLoop):
    ...
    proactive_specialist_tool = make_specialist_request_tool(agent_name, loop)
    ...
    if proactive_specialist_tool is not None:
        all_tools.append(proactive_specialist_tool)
    ...
    try:
        ...
        enable_skills = os.getenv("SWARM_ENABLE_SKILLS", "1").strip().lower() in (
            "1",
            "true",
            "yes",
            "on",
