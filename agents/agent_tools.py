...
def make_communication_tools(agent_name: str):
    ...
    from core.state import _memory as _mem  # noqa: F401 — accessed via global name
...
def make_specialist_request_tool(agent_name: str, loop: asyncio.AbstractEventLoop):
    """Create a proactive specialist-request tool for an agent.

    Unlike report_stuck (which blocks until a specialist finishes), this tool
    fires a non-blocking request: the specialist is spawned into the swarm and
    works in parallel alongside all other agents.  The calling agent continues
    immediately.
    ...
    Returns:
        Strands-decorated tool callable, or None if the stuck queue is
        unavailable.
    """
    if _stuck_queue is None:
        return None
    ...
    def request_specialist(
        ...
        """Proactively spawn a specialist agent that joins the swarm in parallel.

        Use this when you want extra domain expertise working alongside the
        team.  The specialist will be created asynchronously -- it reads and
        writes to the shared blackboard and memory just like any other agent.
        This call returns immediately; you do NOT wait for the specialist.
        """
        request_id = f"{agent_name}_spec_{uuid.uuid4().hex[:8]}"

        _log_event(
            agent_name,
            "PLAN",
            f"Requesting parallel specialist '{specialist_name}': {reason}",
        )
        ...
        return (
            f"Specialist '{specialist_name}' has been requested and will join "
            f"the swarm in parallel.  It will write its results to the "
            f"blackboard -- check there later.  Continue your own work now."
        )

    return _strands_tool(request_specialist, owner=agent_name)
