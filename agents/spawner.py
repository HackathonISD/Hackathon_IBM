...
spawner_loop (background coroutine that listens on the request queue and
...
from agents.agent_tools import make_communication_tools, _strands_tool
...
def create_specialist(
    ...
    Args:
        ...
        context: Optional context or partial work from the requesting agent.
...
async def spawner_loop(loop: asyncio.AbstractEventLoop):
    """Background coroutine that processes specialist requests.

    Listens on the request queue, analyses each request, and delegates to
    specialist agents created via the LLM-powered spawner agent.

    Args:
        loop: The active asyncio event loop.
    """
    _log_event("spawner", "PLAN", "Spawner ready -- listening for specialist requests")
...
Your job: analyze specialist requests from the team and spawn the right temporary specialist agent.
...
2. Call spawn_specialist_tool() with:
   ...
   - tools: subset of ["write_file","read_file","write_blackboard","memory_store","memory_search","run_command"]
    - context: the relevant context from the requesting agent
3. Return the specialist's output directly

You support two request types:
- stuck requests: an agent is blocked and needs problem-solving help
- specialist requests: an agent proactively wants extra domain expertise added to the swarm for this project phase
...
- Domain specialists (data scientist, data engineer, ML engineer, analytics engineer, security specialist...)

If the request is about a data-science or analytics project, strongly consider specialists such as:
- data-scientist
- data-engineer
- ml-engineer
- analytics-engineer
...
Always use the minimal tools needed for the specialist.""",
    ...
    while True:
        ...
        request_kind = request.get("request_kind", "stuck")

        if request_kind == "specialist_request":
            _log_event(
                "spawner",
                "REASON",
                (
                    f"Received specialist request from {request['agent_name']}: "
                    f"{request['specialist_name']}"
                ),
            )

            prompt = f"""
Agent '{request['agent_name']}' wants to add a temporary specialist agent for this project.

Requested specialist: {request['specialist_name']}
Reason: {request['reason']}
Task to assign: {request['task_description']}
Project context: {request['project_context'] if request['project_context'] else 'none'}

Spawn the best-matched specialist, let it do the work, and return a concise result summary.
"""
        else:
            _log_event(
                "spawner",
...
"""
        ...
        try:
            if spawner_agent:
                ...
                request_summary = (
                    request.get("what_i_need")
                    or request.get("specialist_name")
                    or request.get("task_description")
                    or "specialist request"
                )
                result = f"[DEMO] Specialist created for: {request_summary}"
                _log_event(
                    "spawner", "DONE", f"[DEMO] Resolved: {request['agent_name']}"
                )
        except Exception as e:
            result = f"Spawner error: {e}. The requesting agent should try a simpler approach."
